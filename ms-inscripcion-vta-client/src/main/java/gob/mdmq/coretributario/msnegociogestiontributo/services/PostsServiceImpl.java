...
//		this.webClient = webClientBuilder.baseUrl("http://localhost:9992").build();
		//.baseUrl("http://localhost:8080/ms-negocio-gestion-inscripcion-vta").build();
		// Realiza la llamada al microservicio `ms-negocio-gestion-inscripcion-vta`
		this.webClient = webClientBuilder
							.baseUrl("http://localhost:8887/ms-negocio-gestion-inscripcion-vta").build();
	...
	public Flux<PostsDto> obtenerInformacion() {
		return webClient.get()
				...
				.timeout(Duration.ofSeconds(60)) // Tiempo máximo permitido // Tiempo de espera para esta operación
				.onErrorResume(throwable -> {
					System.err.println("Error al realizar la solicitud: " + throwable.getMessage());
					return Flux.empty(); // Retorna un flujo vacío en caso de error
				});
	}
}
